...
if (!function_exists('getClogConfig')) {
    function getClogConfig($key = "", $default = null)
    {
        $realkey = "clog";
...
if (!function_exists('clogPath')) {
    function clogPath()
    {
        return getClogConfig('path');
    }
}
...
if (!function_exists('getRunTime')) {
    ...
    {
        list($usec, $sec) = explode(" ", $request->end_at);
        $end_at = ((float)$usec + (float)$sec);

        list($usec, $sec) = explode(" ", $request->start_at);
        $start_at = ((float)$usec + (float)$sec);

        $request->offsetUnset('start_at');
        $request->offsetUnset('end_at');
